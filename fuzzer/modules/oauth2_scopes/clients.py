...
class Clients:
    ...
    def parse_clients_list(self, clients_list):
        ...
        for client in clients['clients']:
            try:
                self._clients.append(Client(client['client_id'], client['client_secret'], client['scopes']))
            except KeyError:
                logging.log(level=logging.WARNING, msg='{} is missing keys. Skipping'.format(client))
                continue
